...
export default class OrganizationChartWebPart extends BaseClientSideWebPart<IOrganizationChartWebPartProps> {
  ...
  private _injectModernFullWidthStyles(): void {
    ...
    if (this.properties.hideWhiteSpace !== false) {
      rules.push(
        ...
        '  max-width: 100% !important; width: 100% !important; padding: 0px !important; margin: 0px !important;',
        '}',
        '.CanvasSection-xl4 > :first-child { margin: 0px !important; padding: 0px !important; }',
        '[data-automation-id="CanvasControl"] { margin: 0px !important; padding: 0px !important; }',
      );
    }
    ...
    try {
      ...
      if (commentsWrapper?.parentElement) {
  ...
  protected async onInit(): Promise<void> {
    ...
    if (!this.properties.unitField)                     this.properties.unitField = 'Unit';
    if (!this.properties.officeField)                   this.properties.officeField = 'Office';
    if (!this.properties.rankField)                     this.properties.rankField = 'Rank';
    if (!this.properties.nameField)                     this.properties.nameField = 'Name';
    if (!this.properties.billetField)                   this.properties.billetField = 'Billet';
    if (!this.properties.ownerField)                    this.properties.ownerField = 'Owner';
    if (!this.properties.specialtyCodeField)            this.properties.specialtyCodeField = 'Code';
    if (this.properties.officeSortPriority === undefined)       this.properties.officeSortPriority = 'CC,CD,DS,DO,SEL,CCF';
    if (this.properties.unitSortPriority === undefined)   this.properties.unitSortPriority = '';
    if (this.properties.leadershipOffices === undefined)  this.properties.leadershipOffices = 'CC,CD,DS,DO,SEL,CCF';
    if (!this.properties.panelColumns)                  this.properties.panelColumns = '';
    if (this.properties.specialtyCodeColors === undefined) this.properties.specialtyCodeColors = '';
    if (!this.properties.lineColor)                     this.properties.lineColor = 'rgba(170,170,170,1)';
    if (!this.properties.borderColor)                   this.properties.borderColor = 'rgba(68,68,68,1)';
    if (!this.properties.selectedGradientStart)          this.properties.selectedGradientStart = 'rgba(247,107,28,1)';
    if (!this.properties.selectedGradientEnd)            this.properties.selectedGradientEnd = 'rgba(250,217,97,1)';
    if (!this.properties.selectedBorderColor)            this.properties.selectedBorderColor = 'rgba(230,81,0,1)';
    if (!this.properties.selectedFontColor)              this.properties.selectedFontColor = 'rgba(255,255,255,1)';
    if (!this.properties.webpartTitle)                   this.properties.webpartTitle = strings.WebPartTitle;
    if (this.properties.hideWhiteSpace === undefined)    this.properties.hideWhiteSpace = true;
    if (this.properties.hideMenus === undefined)         this.properties.hideMenus = true;
    if (this.properties.hideTopNav === undefined)        this.properties.hideTopNav = true;
  }
  ...
  protected onDispose(): void {
    ...
    if (modernStyle?.parentNode) modernStyle.parentNode.removeChild(modernStyle);
  }
  ...
  private _rgbaToHex(rgba: string): string {
  ...
  private _renderColorField(container: HTMLElement, propKey: ColorPropKey, label: string): void {
  ...
  private _colorField(propKey: ColorPropKey, label: string): IPropertyPaneField<IPropertyPaneCustomFieldProps> {
    return {
      type: PropertyPaneFieldType.Custom,
      targetProperty: propKey,
      properties: {
        key: `${propKey}Picker`,
        onRender: (elem: HTMLElement) => this._renderColorField(elem, propKey, label),
        onDispose: () => undefined,
      },
          ...
          groups: [
            {
              ...
              groupFields: [
                ...
                // Spacer to prevent toggle overlapping text field
                {
                  ...
                  properties: {
                    key: 'layoutSpacer',
                    onRender: (elem: HTMLElement) => { elem.style.height = '8px'; },
                    onDispose: () => undefined,
                  },
                } as IPropertyPaneField<IPropertyPaneCustomFieldProps>,
